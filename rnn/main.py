...
parser.add_argument('--attention_rnn', action='store_true',
                    help='add attention to RNN model')
...
if not args.dry_run:
    with open(Path(args.data) / 'rnn_dictionary.pkl', 'wb') as f:
...
if args.model == 'Transformer':
    ...
    model = model.RNNModel(args.model, ntokens, args.emsize, args.nhid, args.nlayers, args.dropout, args.tied, args.attention_rnn).to(device)
...
def train():
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            logger.info('| epoch {:3d} | {:5d}/{:5d} batches | lr {:02.6f} | ms/batch {:5.2f} | '
...
try:
    for epoch in range(1, args.epochs+1):
        ...
        if not best_val_loss or val_loss < best_val_loss:
            logging.info(f"Saving best: epoch {epoch}")
            ...
            if epoch % args.save_interval == 0:
                ...
                curr_save_path = save_path.parent / f'{save_path.stem}_epoch{epoch}.pt'
