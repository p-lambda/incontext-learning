...
def main():
    ...
    if model_args.eval_incontext:
        model.eval()
        eval_prompts(model, Path(data_args.train_file).parent, tokenizer, results_name=model_args.eval_incontext_results, output_dir=training_args.output_dir)
    ...
    if model_args.generate_cache:
        # terminate early if generating cache
        return {}
